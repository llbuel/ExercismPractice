...
TEST_CASE("equal_tests")
{
    for (equalTest e : equalCases) {
        const auto clock1 = string(date_independent::clock::at(e.c1.hour, e.c1.minute));
        const auto clock2 = string(date_independent::clock::at(e.c2.hour, e.c2.minute));
        if (e.expected) {
            INFO(errorMsg(clock1, clock2, e.msg));
            REQUIRE(clock1 == clock2);
        }
        else {
            INFO("[" << clock1 << " == " << clock2 << "] test case: " << e.msg);
            REQUIRE(clock1 != clock2);
        }
    }
}